...
const getUserPayloadOfCurrentRound = async (game, userId) => {
    ...
    const payload = {
        ...
        playerBefore: game.currentRound === 0 ? null : word.rounds[game.currentRound - 1].player
    };
    if (round.drawing) {
...
const endGame = async (id) => {
    ...
    game.endScreenState = {panel: 0, carouselIndex: 0, guessIsShowing: false}
    await Redis.setItem(game.id, game);
    await notifyUsersOfGame(game.id, 'gameEnded', game);
}

const clickThroughGameEndScreen = async (id, state) => {
    const game = await Redis.getItem(id);
    game.endScreenState = state;
    await notifyUsersOfGame(id, 'endScreenStateChanged', state);
    await Redis.setItem(id, game);
}
